...
/**
 * Created by JKyte on 6/5/2016. This is a base class that contains core functions required for
 * simple brains
 */
public abstract class Brain {

    //  This is initialised when a critter is set
    int[] validFoodOptions;

    public abstract void setCritter(Object critter);

    public abstract void clearCritter();
    ...
    public boolean isFoodInRange(BaseMap map) {
        ...
        for( int ii = 0; ii < map.mapWidth; ii++ ){
            for( int jj = 0; jj < map.mapHeight; jj++ ){
                for( int kk = 0; kk < validFoodOptions.length; kk++){
                    //System.out.println("h: " + ii + " w: " + jj);
                    if( map.getGrid(ii, jj) == validFoodOptions[kk]){
                        return true;
                    }
                }
            }
     ...
     * @returns ActionTarget with coords on the WORLD_MAP
     */
     ...
     *
     * @param visionMap
     * @param validOpts - an int array of valid food options
     * @return
     */
